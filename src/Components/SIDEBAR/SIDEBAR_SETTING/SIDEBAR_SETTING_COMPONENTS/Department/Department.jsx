...
const [users, setUsers] = useState([
  ...
  const [defaultTextdepartmentDropDown, setDefaultTextdepartmentDropDown] =useState("Select Department ");
  ...
  const handleCheckboxClick = (e, userId) => {
    ...
    //---------------depatmentName---------------
    const toggleDropdowndepatmentName = () => {
      setIsDropdownVisibledepartment(!isDropdownVisibledepartment);
      };
    
      const handleDropdowndepatmentName = (depatmentName) => {
        setFormData((prevData) => ({
          ...prevData,
          depatmentName,
        }));
        setDefaultTextdepartmentDropDown(depatmentName);
                  ...
                  {/* -------------depatmentID------------- */}
                    <div className="flex space-x-4">
                      ...
                      <div className="flex flex-col w-1/2 relative">
                        ...
                        <div
                          className="relative"
                          onClick={toggleDropdowndepatmentName}
                          onMouseLeave={() => setIsDropdownVisibledepartment(false)}
                        >
                          <button
                            className="mt-1 p-2 border border-gray-300 rounded-md w-full flex justify-between items-center"
                            id="depatmentNameDropDown"
                            type="button"
                          >
                            {formData.depatmentName || defaultTextdepartmentDropDown}
                            <FaAngleDown className="ml-2 text-gray-400" />
                          </button>
                          {isDropdownVisibledepartment && (
                            <div className="absolute w-full bg-white border border-gray-300 rounded-md top-11 z-10">
                              <ul className="py-2 text-sm text-gray-700">
                                {department.map(({ key, depatmentName }) => (
                                  <li
                                    key={key}
                                    onClick={() => handleDropdowndepatmentName(depatmentName)}
