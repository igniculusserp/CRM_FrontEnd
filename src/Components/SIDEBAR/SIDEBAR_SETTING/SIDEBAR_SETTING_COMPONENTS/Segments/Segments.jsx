...
export default function Segments() {
  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      if (isEditMode) {
        ...
        await axios.put(
          `${protocal_url}${name}.${tenant_base_url}/Admin/segment/edit/${formData.id}`,
          formData,
          config,
        );
  ...
  const EditForm = ({ data, isEditMode }) => {
    const [formData, setFormData] = useState({ id: "", segment: "" });

    useEffect(() => {
      setFormData(data || { id: "", segment: "" });
    }, [data]);
        ...
        {active ? (
          <>
            <div className="flex min-w-screen justify-between items-center flex-wrap gap-5">
              <h1 className="text-3xl font-medium">Segment</h1>
              <button
                onClick={handleAdd}
